...
FORBIDDEN: questions, advice, saying your name. One direct sentence only.${ctx}`;
  ...
  async function saveSession() {
    try {
      ...
      // Write session summary — runs after insert confirmed, row guaranteed to exist
      try {
        ...
        if (scores.factor_sueno     != null) parts.push(`Sleep ${scores.factor_sueno >= 6 ? 'good' : 'poor'}`);
        ...
        if (parts.length > 0) {
          ...
          const { error: sumErr } = await supabase
            ...
            .eq('session_complete', true);
          if (sumErr) {
            console.warn('[saveSession] summary write error:', sumErr.message);
          } else {
            console.log('[saveSession] summary written:', summary);
          }
        }
      } catch (summaryErr) {
...
- Maximum 3 sentences + 1 question. Keep it under 60 words total.`;
  ...
  async function handleAdhocMessage(userText: string) {
    ...
    try {
      ...
      // Turn 3+: hardcoded farewell and close
      if (turn >= 3) {
        const farewell = isES
          ? slot === 'morning'
