  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', letterSpacing: '0.03em' }}>{label}</span>
        <span style={{ fontSize: 11, color: 'white', fontWeight: 600, fontFamily: 'JetBrains Mono, monospace' }}>{value}%</span>
      </div>
...
function DayCard({ day, isToday, idioma, showSexual }: { day: ForecastDay; isToday: boolean; idioma: 'EN' | 'ES'; showSexual: boolean }) {
  const anxietyColor = day.anxiety >= 70 ? '#FF6B6B' : day.anxiety >= 50 ? '#FFD93D' : '#00C896';
  ...
  return (
    <div style={{
      ...
      border: isToday ? '1px solid rgba(255,217,61,0.3)' : '1px solid rgba(255,255,255,0.06)',
      borderRadius: 14,
      padding: '16px 18px',
      marginBottom: 10,
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 10, color: '#4A5568', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 2 }}>
            {isToday ? (idioma === 'ES' ? 'Hoy' : 'Today') : formatDate(day.date, idioma)}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 16 }}>{day.phaseEmoji}</span>
            <span style={{ color: 'white', fontWeight: 600, fontSize: 14, textTransform: 'capitalize' }}>
    ...
    <div style={{ minHeight: '100vh', background: '#0A0A1A', paddingBottom: 80 }}>
      ...
      <div style={{ width: '100%', maxWidth: 430, margin: '0 auto', padding: '52px 24px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
          <div style={{
            background: forecast.mode === 'companion' ? 'rgba(0,200,150,0.12)' : forecast.mode === 'calibration' ? 'rgba(255,217,61,0.12)' : 'rgba(123,97,255,0.12)',
            border: `1px solid ${forecast.mode === 'companion' ? 'rgba(0,200,150,0.3)' : forecast.mode === 'calibration' ? 'rgba(255,217,61,0.3)' : 'rgba(123,97,255,0.3)'}`,
            borderRadius: 6,
            padding: '3px 10px',
            fontSize: 10,
            color: forecast.mode === 'companion' ? '#00C896' : forecast.mode === 'calibration' ? '#FFD93D' : '#7B61FF',
            fontWeight: 600,
      ...
      {forecast.mode === 'learning' && (
        <div style={{ width: '100%', maxWidth: 430, margin: '0 auto', padding: '0 24px 16px' }}>
          <div style={{
            ...
            <div style={{ color: '#7B61FF', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>
      ...
      {forecast.vulnerabilityAlertHours != null && (
        <div style={{ width: '100%', maxWidth: 430, margin: '0 auto', padding: '0 24px 16px' }}>
          <div style={{
            ...
            <div style={{ color: '#FF6B6B', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>
